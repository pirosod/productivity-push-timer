...
@onready var _today_table: VBoxContainer = $MainScroll/Content/TodaySection/TodayTable
...
func _apply_window_settings() -> void:
	...
	var width: int = int(settings.get("width", UiScale.scale_i(380)))
	var height: int = int(settings.get("height", UiScale.scale_i(420)))
	if width < 600 and height < 700:
		width = UiScale.scale_i(380)
		height = UiScale.scale_i(420)
...
func _refresh_today_table() -> void:
	...
	_today_table.build_for_day(today_key, true)
